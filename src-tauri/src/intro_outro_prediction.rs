...
) -> Result<Vec<VideoDetails>, String> {
...
fn get_video_chapters(video_path: &Path) -> Result<Vec<(f64, f64)>, String> {
    ...
    if let Some(chapter_list) = chapters["chapters"].as_array() {
        let chapter_times = chapter_list.iter().filter_map(|chapter| {
...
fn detect_intro_outro(video_duration: f64, chapters: &[(f64, f64)]) -> (String, String, String, String) {
    ...
    for (chapter_start, chapter_end) in chapters.iter() {
        ...
        if *chapter_start < half_duration && duration >= 50.0 && duration <= 180.0 {
            intro_start = format_duration(*chapter_start);
            intro_end = format_duration(*chapter_end);
        }

        if *chapter_start > half_duration && duration >= 50.0 && duration <= 180.0 {
            outro_start = format_duration(*chapter_start);
            outro_end = format_duration(*chapter_end);
        }
    }
